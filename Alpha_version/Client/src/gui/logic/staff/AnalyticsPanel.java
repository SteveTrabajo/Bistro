...
public class AnalyticsPanel {
    ...
    @FXML 
    private Label distributionTitleLabel;
    @FXML 
    private Label distributionSubtitleLabel;
    @FXML 
    private BarChart<String, Number> arrivalBarChart;
    @FXML 
    private LineChart<String, Number> reservationsLineChart;
    @FXML 
    private Label totalOnTimeLabel;
    @FXML 
    private Label totalLateLabel;
    @FXML 
    private VBox peakTimesBox;
    @FXML 
     ...
     * Loads report types and available months.
     */
    ...
    /**
     * Loads available months for the selected report type.
     * Populates monthCombo and selects previous month if available.
     * Triggers report load if a month is selected.
     */
    ...
    /**
	 * Loads the report for the selected month and report type.
	 * Updates the dashboard upon retrieval.
	 */
    @FXML
    private void onLoadReport() {
        ...
        new Thread(() -> {
            BistroClientGUI.client.getMonthlyReportsCTRL().requestReport(reportTypeCombo.getValue(), year, month, false);
    ...
    /**
	 * Updates all dashboard elements based on the provided report data.
	 *
	 * @param r report data
	 */
    ...
    /**
	 * Updates the top bar chart to reflect arrival times or reservation types.
	 *
	 * @param r report data
	 */
    ...
    /**
	 * Updates the middle line chart to show daily trends.
	 *
	 * @param r report data
	 */
    ...
    private void addSeries(String name, Map<Integer, Integer> data) {
		...
		if (data != null && !data.isEmpty()) {
			data.entrySet().stream().sorted(Map.Entry.comparingByKey())
					.forEach(e -> s.getData().add(new XYChart.Data<>(String.format("%02d", e.getKey()), e.getValue())));
		}
    ...
    /**
     * Updates the bottom distribution section based on report type.
     * 
     * @param r
     */
    ...
    /**
	 * Populates distribution rows from a string-keyed map.
	 *
	 * @param map label -> count
	 */
    ...
    /**
	 * Sums the values in an integer map.
	 *
    ...
    private void prepareDayAxisFromCombo() {
        ...
        int year = Integer.parseInt(ym.substring(0, 4));
        int month = Integer.parseInt(ym.substring(5, 7));
        CategoryAxis x = (CategoryAxis) reservationsLineChart.getXAxis();
    ...
    /**
     * Renames lateness buckets (data collection of all the late arrivals) to user-friendly labels.
     *
