...
public class RestaurantManagementPanel {
		...
		for (int i = 0; i < 7; i++) {
			...
			if (activeChecks.get(i).isSelected()) {
				...
				// VALIDATION 2: Shift must be at least 2 hours
					...
					showAlertInfo("Invalid Hours",
							"Day " + dayId + ": The restaurant must be open for at least 2 hours.");
					hasErrors = true;
				} else {
	...
	void btnAddHoliday(ActionEvent event) {
		...
		if (!isClosed) {
			...
			if (openStr == null || closeStr == null) {
				showAlertInfo("Invalid Hours",
						"Please select open and close times for this holiday (or mark it as closed).");
				return;
			}
		...
		Holiday holiday = new Holiday(dpHoliday.getValue(), txtHolidayName.getText().trim(), isClosed, openTime,
				closeTime);
		...
		// NEW: clear holiday time pickers
		if (cmbHolidayOpen != null)
			cmbHolidayOpen.getSelectionModel().clearSelection();
		if (cmbHolidayClose != null)
			cmbHolidayClose.getSelectionModel().clearSelection();
