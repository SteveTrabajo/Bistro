...
public class WaitingListService {
	...
	public WaitingListService(BistroServer server, BistroDataBase_Controller dbController, OrdersService ordersService,
			TableService tableService, ServerLogger logger, UserService userService) {
	 ...
	 * @param addToWaitlist      True if adding to waitlist, false for immediate
	 *                           seating.
	 * @param calculatedWaitTime Estimated wait time in minutes (if adding to
	 *                           waitlist).
	...
	public Object checkAvailabilityAndSeat(int dinersAmount, int userID) {
		...
		// In case seating is NOT possible, calculate estimated wait time and return
		// WaitListResponse
		long waitTime = calculateEstimatedWaitTime(dinersAmount);

		// We must still create the WaitList order in the DB before returning
		String code = createWaitListOrder(dinersAmount, userID, true, (int) waitTime);

		String msg = "No table available. Estimated wait: " + waitTime + " min.";

		// Return the object that the client is now receiving
	...
	public Object handleMemberWalkIn(int dinersAmount, String memberIdStr) {
		...
		loginData.put("memberCode", Integer.parseInt(memberIdStr));
	 ...
	 */
	/**
	 * Shared logic for seating or waitlist registration. Returns a Map with the
	 * results.
	 */
	private Object processSeatingOrWaiting(int dinersAmount, int userID) {
		...
		if (availability instanceof Map) {
			// SCENARIO A: Seated immediately.
			// Keep this as is if your Client expects a Map for success.
			return availability;
			...
			if (code != null) {
				// IMPORTANT: Instead of making a new Map, just update the response object
				// and return it. This prevents the casting error on the Client.
				return res;
			}
		}
		return null;
